...
| Recruiting board | 247Sports (football/men's and women's basketball), Perfect Game (baseball) | Source-reported commitments for the active recruiting cycle, with profile information and ratings/rankings only when provided |
| Recruiting offers | 247Sports public offer lists | Football and men's and women's basketball historical offers, separate from commitments; an offer does not establish current availability |

Baseball offer lists report `unavailable` with `provider-does-not-cover-offers`. Women's basketball uses 247Sports because ESPN HoopGurlz returned HTTP 202 from GitHub runners. Its provider coverage is incomplete: empty commitment/offer listings use `empty` with `provider-has-no-commitment-records` or `provider-has-no-offer-records`, not a verified zero-player class or offer ledger. Nonempty women's lists also carry `provider-reported-records-coverage-incomplete`. Display the mapper's coverage label, and preserve prior same-source nonempty data as stale when the source later lists no records. All collections expose their actual coverage and source health. This repository does not claim a complete recruiting offer ledger, transfer portal database, historical archive, or real-time scoreboard.
...
The 36-source catalog includes Sports Illustrated's 16 school feeds, three ESPN public news JSON feeds, NCAA.com, CBS Sports, Heartland College Sports, and ten team-focused/student publications. ESPN's website RSS returned HTTP 202 from GitHub runners; the news API uses the same public API host as rosters and schedules. Shared feeds are fetched once per complete run. Missing photos may be filled from the original article's verified public image metadata, with at most three article fetches per sport per school per run. No article bodies are republished.
